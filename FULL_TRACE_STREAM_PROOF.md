...
**System:** Pravah  
**Claim:** For a single trace_id, the stream shows all 6 required stages in order, each as an independent flat signal. No stage missing. No stage inferred.
...
# Stages 2-6 — Sarathi decision triggers all remaining stages
...
## Full Live Stream Output — All Signals for This Trace
...
| # | Required Stage | Signal Type | Present | emitted_at |
|---|---------------|-------------|---------|------------|
...
All 6 stages present. All same trace_id. All flat. No inference. No missing stage.
...
- No `signals[]` array wrapper: ✅
- Each `data:` line = exactly one signal: ✅
- Every signal carries `trace_origin`, `source`, `trace_hash`: ✅